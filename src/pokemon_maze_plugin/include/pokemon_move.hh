...
#ifndef GAZEBO_MODEL_POKEMON_MOVE_HH_
#define GAZEBO_MODEL_POKEMON_MOVE_HH_
...
namespace gazebo
{
  ...
  class ModelMove : public ModelPlugin
  {
    ...
    private: void Move(const ignition::math::Vector3d &_start,
                       ...
                       ignition::math::Quaterniond &_Rotation);
    ...
    public:  
          float GenRandom(void)
                {                 
                  float random;
                  srand(time(NULL));//设置随机数种子，使每次产生的随机序列不同
                  random =  rand() % 10 /(float)10 ;
                  return random;
                }
    ...
    private: ignition::math::Quaterniond startQuater;
