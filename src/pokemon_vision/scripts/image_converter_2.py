...
def callback(img_msg, box_msg):
    ...
    if len(boxes_list) > 1:
        ...
        image_name = "/tb3_2" + timestr + ".jpg" #图像命名：时间戳.jpg
        cv2.imwrite(save_image_path + image_name, rec_img)  #保存；
        print("save tb3_2 successfully!")
        # cv2.imshow("frame" , cv_img)
        # cv2.waitKey(3)
 
if __name__ == '__main__':
    rospy.init_node('save_pokemon', anonymous=True)
    sub_image = message_filters.Subscriber("/tb3_2/camera/image", Image)
    sub_box = message_filters.Subscriber("tb3_2/darknet_ros/bounding_boxes", BoundingBoxes)
    # 设置时间同步器 fs等待消息[sub_image, camera_info_sub]，queue_size 10
    print("wait for tb3_2...")
    sync_listener = message_filters.TimeSynchronizer([sub_image, sub_box], 1)
    # 设置回调函数
